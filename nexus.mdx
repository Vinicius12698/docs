...
| `package_id` | integer | Sim | ID do pacote Nexus |
| `resale_id` | integer | Sim | ID da revenda |
| `bandwidth_limit` | integer | Não | Limite de banda em Mbps (padrão: 100) |

### Exemplo de Requisição

```bash
curl -X POST "https://api.paineloffice.com/nexus" \
...
```bash
curl -X POST "https://api.paineloffice.com/nexus/extend/3001" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer {token}" \
  -d '{
    "extend_days": 30,
